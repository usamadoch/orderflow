...
**Resize handling:** when the canvas's actual pixel dimensions change (window resize, sidebar toggle), the viewport's price/time _ranges_ should stay the same — only the pixel dimensions used in the conversion functions change. Getting this backwards (resetting the viewport on resize) is a common and jarring bug — the chart appears to "jump" to a different price range whenever the window is resized.
