            ...
            <div className="mb-4 space-y-3 p-3 rounded-lg border border-border bg-background/50">
              ...
              <div>
                <div className="flex justify-between text-xs font-medium mb-1">
                  <span className="text-text-dim">TimescaleDB (Market History)</span>
                  <span className="text-main">{databases.timescale.usedMb} / {databases.timescale.totalMb} MB</span>
                </div>
                <div className="h-1.5 w-full bg-border rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-accent transition-all"
                    style={{ width: `${Math.min(100, (databases.timescale.usedMb / databases.timescale.totalMb) * 100)}%` }}
                  />
                </div>
              </div>
            </div>
