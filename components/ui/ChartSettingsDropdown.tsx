...
export function ChartSettingsDropdown({ panelId, onClose }: ChartSettingsDropdownProps) {
  ...
  return (
    <div
      ref={dropdownRef}
      className="absolute top-12 right-4 w-80 bg-[#0D0D0D]/95 backdrop-blur-xl border border-[#1F1F1F] rounded-xl shadow-2xl z-50 flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200"
      style={{ maxHeight: 'calc(100vh - 100px)' }}
    >
      {/* Header */}
      <div className="p-4 border-b border-[#1F1F1F] flex items-center justify-between bg-[#080808]/50">
      ...
      {/* Tabs */}
      <div className="flex px-1 pt-1 bg-[#080808]/30 border-b border-[#1F1F1F]">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex-1 py-2 text-[10px] font-black uppercase tracking-wider transition-all duration-200 border-b-2 ${activeTab === tab.id
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        <div className="flex flex-col gap-6">
          ...
          {activeTab === 'chart' && (
            <>
              {/* Bucket Size (Moved to Chart Settings) */}
              <div className="space-y-3">
                <div className="text-[10px] font-bold text-text-dim uppercase tracking-wider">Aggregation</div>
          ...
          {activeTab === 'signals' && (
            <>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="text-[10px] font-bold text-text-dim uppercase tracking-wider">Absorption Signals</div>
                  <button
